...
// Slider (Kaydırma) İşlevi
function moveSlide(sliderClass, step) {
    // İlgili slider'ın içindeki tüm görselleri bul
    let images = document.querySelectorAll(`.${sliderClass} img`);
    let currentIndex = 0;

    // Şu an aktif olan (görünen) görselin sırasını bul
    for(let i = 0; i < images.length; i++) {
        if(images[i].classList.contains('active')) {
            currentIndex = i;
            images[i].classList.remove('active'); // Aktifliği al
            break;
        }
    }

    // Yeni sırayı hesapla (Başa veya sona dönmesini sağla)
    let newIndex = currentIndex + step;
    if (newIndex >= images.length) newIndex = 0;
    if (newIndex < 0) newIndex = images.length - 1;

    // Yeni görsele active class'ını ver
